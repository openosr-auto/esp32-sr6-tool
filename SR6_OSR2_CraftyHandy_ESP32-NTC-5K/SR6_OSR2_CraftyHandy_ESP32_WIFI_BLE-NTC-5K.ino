...
#define FIRMWARE_ID "SR6-OSR2-CraftyHandy-NTC-5K v2.0 温控开启(NTC34/LED32)"
#define TCODE_VER "TCode v0.3"

// 修改：NTC温度控制设置（5K 3470型号）
#define NTC_PIN 34           // NTC热敏电阻 -> ADC1_CH6 (GPIO 34)
#define ALARM_LED_PIN 32     // 报警LED -> GPIO 32
#define R_REF   10000.0f     // 分压电路中的参考电阻阻值(10KΩ) — PCB固定电阻，未更换
#define NTC_R0  5000.0f      // NTC在25°C时的标称阻值(5KΩ)
#define NTC_B   3470.0f      // NTC的B值改为3470K（5K 3470型号）
...
void temperatureSafetyCheck() {
    ...
    if (currentTemp < -10.0 || currentTemp > 120.0) {
      ...
      if (motionPaused) { motionPaused = false; digitalWrite(ALARM_LED_PIN, LOW); }
      return;
    }
